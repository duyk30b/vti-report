...
export class SyncItemService {
	...
	// 	const ticketImports = await this.natsClientItemService.getWarehouseImportList({ confirmedTime: [startTime, endTime] })
	...
	// 	await this.itemRepository.deleteMany({ timeSync: new Date(daySyncString) })
	// 	await this.itemRepository.insertMany(warehouseImports)
	// }
}